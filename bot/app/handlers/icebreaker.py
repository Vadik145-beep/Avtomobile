...
async def _run_icebreaker(tg_id: int, reply_fn) -> None:
    await reply_fn("⏳ Запускаю лидогенерацию...")
    ...
    if result.balance_empty:
        await reply_fn(
            ...
            "После пополнения нажмите <b>«Запустить лидогенерацию 🚗»</b> снова.",
            parse_mode="HTML",
            reply_markup=main_menu_keyboard(icebreaker_active=False),
        )
    elif result.dispatched == 0:
        await reply_fn(
            "✅ <b>Лидогенерация активирована!</b>\n\n"
...
async def _stop_icebreaker(tg_id: int, reply_fn) -> None:
    ...
    await reply_fn(
        "🛑 <b>Лидогенерация остановлена.</b>\n\n"
        "Вы больше не будете получать новые лиды.\n"
        "Чтобы возобновить — нажмите «Запустить лидогенерацию 🚗».",
        parse_mode="HTML",
        reply_markup=main_menu_keyboard(icebreaker_active=False),
    )
